...
- 📱 **Удалённый доступ** - управление через Android Termux и Ubuntu клиент
...
## 🖥️ Настройка Ubuntu клиента

### Автоматическая настройка

```bash
# Скачайте скрипт настройки клиента
wget https://raw.githubusercontent.com/Redrock453/agenticseek-ubuntu/main/setup_ubuntu_client.sh
chmod +x setup_ubuntu_client.sh
./setup_ubuntu_client.sh
```
...
# Установите необходимые пакеты
sudo apt update && sudo apt upgrade -y
sudo apt install -y openssh-client git python3 tmux curl wget nano htop

# Сгенерируйте SSH ключи
ssh-keygen -t ed25519 -C "ubuntu-client@$(hostname)"

# Скопируйте ключ на сервер
ssh-copy-id ubuntu@your_server_ip
```

### Создание скриптов подключения

```bash
# Создайте скрипт подключения
nano ~/connect_agenticseek.sh
```
...
### Управление через aliases

Добавьте в `~/.bashrc`:
```bash
# AgenticSeek aliases
alias agenticseek="~/connect_agenticseek.sh"
alias agenticstatus="ssh -t ubuntu@your_server_ip 'sudo systemctl status agenticseek'"
alias agenticlogs="ssh -t ubuntu@your_server_ip 'sudo journalctl -u agenticseek -f'"
```

### Графический интерфейс

После выполнения скрипта настройки у вас появятся:
- 🖱️ **Ярлык на рабочем столе** "Connect to AgenticSeek"
- 📋 **Меню управления** через команду `agentic_menu`
- 🔧 **Утилиты управления** в `~/agenticseek_tools.sh`

### Команды управления

```bash
# Подключиться к AgenticSeek
...
# Перезапустить
~/agenticseek_tools.sh restart

# Обновить
~/agenticseek_tools.sh update

# Создать бэкап
~/agenticseek_tools.sh backup
```
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Ubuntu Client │    │   Android       │    │   Ubuntu Server │
│   (Desktop)     │◄──►│   (Termux)      │◄──►│   (SSH/Tmux)    │
└─────────────────┘    └─────────────────┘    └─────────────────┘
                                                       │
                                              ┌────────┴────────┐
                                              ▼                 ▼
